...
def autoencoder_fn(x, dimensions, reuse=False):
    ...
    encoder_dims = dimensions[1:]
    decoder_dims = dimensions[:-1][::-1]

    # %% Build the encoder
    encoder = []
    with tf.variable_scope('ENCODER', reuse=reuse):

        for layer_i in range(1, len(dimensions)):
            layer_prev = layer_i - 1
            n_input = dimensions[layer_prev]
            ...
            name = "OUTPUT_%d" % (len(dimensions) - layer_i - 1)
